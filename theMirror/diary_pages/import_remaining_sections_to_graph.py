...
CSV_PATHS = [
    Path("data/deploying_ai.csv"),
    Path("data/using_ai.csv"),
    Path("data/governing_ai.csv"),
    Path("data/resisting_ai.csv"),
    Path("data/second_order_ai_impact.csv"),
]
...
def unique_rows(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        grouped[(row["branch"], row["sub_branch"], row["region"], row["entity"])].append(row)
    out: list[dict] = []
...
def main() -> int:
    ...
    subbranch_ids: dict[tuple[str, str], int] = {}
    region_ids: dict[str, int] = {}

    for row in rows:
        branch = row["branch"]
        subbranch = row["sub_branch"]
        key = (branch, subbranch)
        if key in subbranch_ids:
            continue
        node = find_node(nodes, subbranch)
        if node is None:
            raise RuntimeError(f"Missing sub-branch node: {branch} / {subbranch}")
        nid = node_id(node)
        if nid is None:
            raise RuntimeError(f"Sub-branch missing id: {branch} / {subbranch}")
        subbranch_ids[key] = nid
