...
SUBBRANCHES = [
    ("foundation model labs", "building the frontier labs that train the most capable models"),
    ("hardware & chips", "accelerators, foundries, and custom silicon that make modern AI possible"),
    ("cloud infrastructure", "the cloud layer that trains, serves, and scales AI systems"),
    ("data", "datasets, labeling, curation, and synthetic data"),
    ("tooling & frameworks", "frameworks, developer platforms, and inference tooling"),
    ("open source models", "model families and communities distributing open weights"),
    ("interpretability", "trying to understand what is happening inside learned systems"),
    ("alignment & safety", "making capable systems safer, more reliable, and more controllable"),
    ("benchmarking & evaluation", "measuring capability, reliability, and task performance"),
    ("ML theory", "the mathematics and conceptual foundations underneath machine learning"),
    ("science communication", "people and media making AI legible to the wider world"),
]
...
def main() -> int:
    ...
    main_node = find_node(nodes, "making & understanding AI")
    if main_node is None:
        raise RuntimeError('Could not find existing "making & understanding AI" node.')
    main_id = node_id(main_node)
    if main_id is None:
        raise RuntimeError('Main node missing id.')

    created_nodes = 0
    created_edges = 0

    for name, note in SUBBRANCHES:
        tags = ["making-understanding-ai", "sub-branch", slug(name)]
        ...
        if sig not in edge_set:
            request_json(
                ...
                api_url("/edges"),
                {
                    ...
                    "confidence": 0.9,
                    "created_by": "manual",
                    "evidence": f'{name} is part of the making & understanding AI branch.',
                },
            )
            edge_set.add(sig)
            created_edges += 1
